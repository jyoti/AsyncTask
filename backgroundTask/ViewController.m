...
- (void)viewDidLoad {
    ...
    NSURLSessionConfiguration *sessionConfiguration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:@"com.backgroundTask"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // Set the file title.
    cell.lblLink.text = fdi.fileTitle;
    
    [cell.btnDownload addTarget:self action:@selector(onDownload:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)onDownload:(id)sender {
    ...
    if ([[[sender superview] superview] isKindOfClass:[ViewControllerCell class]]) {
        // Get the container cell.
        ViewControllerCell *containerCell = (ViewControllerCell *)[[sender superview] superview];
...
-(void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite{
    ...
    if (totalBytesExpectedToWrite == NSURLSessionTransferSizeUnknown) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            ...
            ViewControllerCell *cell = [self.tblFiles cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
            cell.progressView.progress = fdi.downloadProgress;
        }];
    }
}
